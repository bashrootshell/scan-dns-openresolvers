...
''' check: if there's an argument / if IP address is larger than a /32.
    if not, there's only 1 IP Address in the list '''
...
try:
    ...
    if len(argv) == 1:
        ...
        iplist = [ipaddr for ipaddr in ip_network
                  (argv[1]).hosts()]  # list comprehension to get all hosts
...
for host in iplist:
    ...
    try:
        ...
        with newsocket.socket(
                newsocket.AF_INET, newsocket.SOCK_DGRAM) as packet:
            ...
            if (b'\xc0\x0c\x00\x01\x00\x01' and
                ...
                nixtime = int(time())
                print(f'{host},open,{nixtime}')

    except newsocket.timeout:   # timeout generated by network conditions
        continue
